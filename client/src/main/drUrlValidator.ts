// DR NAS warm-failover LAN-target validator (Electron MAIN process only — no
// Electron deps, no renderer access). Trusted-side gate for the Direct/LAN
// `baseUrl` the till re-authenticates against during a WAN outage.
//
// Ported FAITHFULLY from aeris_client/mobile/src/constants/config.ts
// (isLocalUrlSafeForCache + helpers rawHostFromUrl, rawIpv4HostAcceptable,
//  isPrivateRfc1918, isDockerBridge, mappedV4) and kept behaviourally
// identical to the v1 port at archive/desktop-v1/src/dr-url-validator.js.
//
// The Direct `baseUrl` becomes the in-store (NAS) target the renderer/relay
...
// strictly than a lax http/https check (PROJECT_DR_NAS_WARM_FAILOVER.md
// §15 / §14.7 / §17.4 / §22):
//
...
/**
 * Strict gate for the DR Direct/LAN `baseUrl` (PROJECT_DR_NAS_WARM_FAILOVER.md
 * §15-2 / §22.5 Q6). Returns true only for an https:// URL whose host is a
 * private LAN target (RFC1918 minus Docker bridges, or a single-label `.local`
 * name). Everything else — loopback, public hosts, alt-encoded IPv4,
 * credentials-in-URL, plain http — is rejected. Mirrors the mobile + v1 copies.
 */
